...
#imports for YOLO model library
#import sys
#sys.path.insert(0, '/home/snadipal/catkin_workspace/src/hackathon/')
import libtest as yolo
...
def get_traffic_symbol(net,meta,img):
        #print("get_traffic_symbol")
        #time.sleep(5)
        r = yolo.detect(net, meta, img)
        return r
...
def threaded_image_handling(cv2_img):
        global turn, recognition_running, obj_dist
        #print("Processing Image data", obj_dist)
        cv2.imwrite('camera_img.jpeg', cv2_img) ## save image
        ##call your recognition model here and set the value of turn

        global h
        #Calling YOLO model to classify the image and get road sign!!!
        if use_YOLO == True:
          h = get_traffic_symbol(net,meta,'camera_img.jpeg')
          turn = h         
        else : 
...
def image_callback(msg):
        #print("Received an image!", recognition_running)
        global recognition_running, laser_running
        # if laser_running == False:
        #       thread.start_new_thread(threaded_line_follower, (cv2_img,))
        
        if recognition_running == True or laser_running == True:
                ...
                if turn=="na":
                        ...
                        try:
                                ...
                                #print("Starting an image handling thread {%d , %d} ", .format(obj_dist,inf_thr))
                                if (obj_dist < inf_thr ):
                                        recognition_running = True
...
def laser_callback(msg):
        ...
        global laser_running, deviation_running, obj_dist
        # print(msg)
...
if __name__ == '__main__':
        try:
                ...
                linear_vel = 0.2                                ## linear velocity of bot
